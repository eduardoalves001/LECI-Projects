...
static unsigned long solution_2_count; // effort dispended solving the problem after better solution
static int bestnmoves[_max_road_size_ + 1][_max_road_speed_+1];
...
{
  int i, new_speed;
  // int t_travagem = ((speed-1)*(speed-2))/2;
  //solution_2_count++;
  ...
  for(new_speed = speed + 1;new_speed >= speed - 1;new_speed--){
    ...
    if(new_speed >= 1 && new_speed <= _max_road_speed_ && position + new_speed <= final_position)
    {
      ...
      for(i = 0;i <= new_speed && new_speed <= max_road_speed[position + i];i++);
      // if (final_position - position > t_travagem){
      // valores_anteriores[i] = n.moves;
      
      //solution_2_best.positions[move_number] = solution_2.positions[move_number] + new_speed;

      if(i > new_speed){  // && move_number +1 < solution_2_best.n_moves ){       
...
static void solve_2(int final_position)
{
  ...
  for(int i = 0; i<= final_position; i++){
    for(int j = 0; j<= _max_road_speed_; j++){
      bestnmoves[i][j] = final_position + 100;
    }
  }
