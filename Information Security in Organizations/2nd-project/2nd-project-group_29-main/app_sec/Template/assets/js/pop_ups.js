...
function submit(user_id) {
    ...
    xhr.onreadystatechange = () => {                                            
        ...
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            // Request finished. Do processing here.
            response = JSON.parse( xhr.response ); 
            if (response.status == true) alert("Mensagem enviada com sucesso.")
        ...
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            ...
            if (response.status == true) {
                for(i=0; i<response.result.length; i++) {
                    ...
                    //subject.innerHTML = '*'+response.result[i]["user"]+'*'+" wrote on "+"'"+response.result[i]["subject"]+"':"    //inseguro
                    subject.textContent = '*'+response.result[i]["user"]+'*'+" wrote on "+"'"+response.result[i]["subject"]+"':"    //seguro
                    subject.setAttribute("style", "color: lime")

                    var message = document.createElement("h5")
                    //message.innerHTML = response.result[i]["comment"]         //inseguro
                    message.textContent = response.result[i]["comment"]           //seguro
